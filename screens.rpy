    ...
    if language_game == 1:
        ...
        use game_menu(_("Об игре"), scroll="viewport"):
            ...
            vbox:
                ...
                text _("Данная игра является некоммерческим продуктом. Все файлы, используемые в оной, являются собственностью их создателей, а разработчики, создающие игру, не пытаются присвоить себе их творчество. Если вы против того, чтобы в данной игре использовался ваш арт/фон/музыка и т.д., то обратитесь к разработчикам для дальнейшего обсуждения этой проблемы.")
                text _("Благодарим сайт icons8.com за предоставленные материалы.")
                text _("Поддержать разработчиков вы можете по ссылке https://www.patreon.com/school123.")
    elif language_game == 2:
        ...
        use game_menu(_("Об игре"), scroll="viewport"):
            ...
            vbox:
                ...
                text _("This game is a non-profit product. All files used in it are the property of their creators, and the developers who create the game do not try to appropriate their creativity. If you are opposed to using your art / background / music, etc. in this game, then contact the developers for further discussion of this problem.")
                text _("We thank icons8.com for the material provided.")
                text _("You can support the developers at https://www.patreon.com/school123.")
...
screen confirm(message, yes_action, no_action):
    ...
    if reaction_gid >= 5:
        $ reaction_gid = 4
    if reaction_gid <= 4:
        add "gui/overlay/reaction_gid/reaction"+str(reaction_gid)+".png" at text_p
    zorder 200
    style_prefix "confirm"
    imagemap at text_p:
        ground "gui/overlay/confirm.png"
        hover "gui/overlay/confirm_hover.png"
        hotspot (1071, 646, 93, 72):
            if persistent.message_no == True:
                action SetVariable('persistent.message_no', 10), no_action
            else:
                action no_action
        hotspot (975, 649, 94, 67):
            if reaction_gid <= 4:
                unhovered SetVariable('reaction_gid', reaction_gid+1)
            if persistent.message_no == True:
                action SetVariable('persistent.message_no', 10), yes_action
            else:
                action yes_action
    if persistent.message_no == False:
        imagebutton xalign 0.42 yalign 0.62:
            idle "gui/overlay/message_no_idle.png"
            action SetVariable("persistent.message_no", True)
    else:
        imagebutton xalign 0.42 yalign 0.62:
            idle "gui/overlay/message_no_hover.png"
            action SetVariable("persistent.message_no", False)
    ## Правый клик и esc, как ответ "Нет".
    key "game_menu" action no_action
    timer 2 repeat True action [If(reaction_gid >= 4, true=yes_action)]
    if persistent.message_no >= 10:
        timer 0.000001 repeat True action [If(persistent.message_no >= 1, true=yes_action)]
